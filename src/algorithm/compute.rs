...
use game::{Stone, check_double_free_threes, JOINED_ACTIONS, get_alignment};

fn get_alignement_point(board: &Vec<Vec<u8>>, line: usize, col: usize, board_size: usize, player: u8) -> i32 {
    let mut points = 0;
    JOINED_ACTIONS.iter().for_each(|actions| {
        points  += get_alignment(
            board,
            &Stone(line, col),
            player,
            board_size,
...
fn get_basic_point(line: usize, col: usize, board_size: usize) -> i32 {
    ...
    if !point_line && !point_col {
        return  (board_size - line + board_size - col )as i32;
    } else if !point_line{
        return (board_size - line + col) as i32;
    }else if !point_col{
        return (board_size - col + line) as i32;
    } else {
         return (line + col) as i32;
    }
}
...
pub fn compute(board: &Vec<Vec<u8>>, player: &u8, board_size: usize) -> (usize, usize) {
    ...
    for (i_line, line) in board.iter().enumerate() {
        for (i_col, col) in line.iter().enumerate() {
            if *col == 0 {
                if check_double_free_threes(&board, &Stone(i_line, i_col), *player, board_size) {
                    //forbidden.push([i_line, i_col]);
                } else {
